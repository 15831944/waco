...
Controller::~Controller()
{
	try
	{
		...
		Entity Ent;
		Acad::ErrorStatus	es;
		//Crash/Hang bug code - deletes entities
...
void Controller::SetGlobalsToDefault()
{
	...
	std::string szPath = inireader.ReadString("FILEPATH", "STOCK_LIST", "");
	//LPWSTR str = ConvertToLPCSTR(szPath);
	CString str(szPath.c_str());
	//LPCTSTR str=_T("C:\\ProgramData\\Autodesk\\ApplicationPlugins\\KwikScaf.bundle\\Contents\\Excel\\Stock.csv");
	SetStringInRegistry(PROJECT_STOCK_LIST_FILENAME_SECTION, str);
	
	szPath = inireader.ReadString("FILEPATH", "COMPONENT_DETAILS", "");
	CString strC(szPath.c_str());
	//str = ConvertToLPWSTR(szPath);
	//str=_T("C:\\ProgramData\\Autodesk\\ApplicationPlugins\\KwikScaf.bundle\\Contents\\Excel\\ComponentDetails.csv");
	SetStringInRegistry(PROJECT_COMPONENT_DETAILS_FILENAME_SECTION, strC);
	
	szPath = inireader.ReadString("FILEPATH", "DRAWING_DETAILS", "");
	CString strD(szPath.c_str());
	//str = ConvertToLPWSTR(szPath);
	//str=_T("C:\\ProgramData\\Autodesk\\ApplicationPlugins\\KwikScaf.bundle\\Contents\\Excel\\DrawingDetails.csv");
	SetStringInRegistry(PROJECT_DRAWING_DETAILS_FILENAME_SECTION, strD);
	////////////////////////////////////////////////////////////
...
void Controller::Serialize(CArchive &ar)
{
	...
	{
		//AfxMessageBox(_T("Start Serialize"));
#ifdef _DEBUG
...
#endif //_DEBUG
		//AfxMessageBox(_T("Start Serialize2"));
		...
		if( !IsInsertingBlock() )
		{
			...
			m_bShowFirstScale	= false;
			m_bShowScaleLines	= false;
			//AfxMessageBox(_T("insert block2"));
			m_iaInvisibleMatrixElement.RemoveAll();
			//AfxMessageBox(_T("insert block3"));
			m_daLevels.RemoveAll();
			//AfxMessageBox(_T("insert block4"));
			SetIsOpeningDocument(true);
			if( IsDeleteSchematic() )
			{
				//AfxMessageBox(_T("insert block5"));
				//Crash/Hang bug code - TempEntity not initialized - deletes entities
				try
				{
				...
				{
					//AfxMessageBox(_T("insert block55-catch"));
				}
				//AfxMessageBox(_T("insert block6"));
			}
			else
			{
				//AfxMessageBox(_T("insert block7"));
				acutPrintf( _T("\nThe schematic will not be deleted during this load, type 'DS' to change this setting!") );
				//AfxMessageBox(_T("insert block8"));
			}
			SetIsOpeningDocument(false);
			//AfxMessageBox(_T("insert block9"));
			CreateLayers();
			daRLs.RemoveAll();
			SetUseActualComponents( UAC_NEITHER );
			m_ia3DBaysSelected.RemoveAll();
...
*/
			//AfxMessageBox(_T("end insert block"));
		}
    ...
    acutPrintf( _T("\nKwikScaf Load Lap Time = %i:%02i"), TimeTaken.GetMinutes(), TimeTaken.GetSeconds() );
      ...
      acutPrintf( _T("\nKwikScaf Load Runs Time = %i:%02i"), TimeTaken.GetMinutes(), TimeTaken.GetSeconds() );
		...
		case CONTROLLER_VERSION_1_0_0 :
			...
			// End BOM Extra Component List Retrieval
			//AfxMessageBox(_T("End BOM Extra"));
			ar >> iNumberOfLapboards;
			for( iLapboardNumber=0; iLapboardNumber < iNumberOfLapboards; iLapboardNumber++ )
			{
				pLapboard = new LapboardBay();
			...
			if (bViewMatrixCreated)
			{
				if( GetMatrix()!=NULL )
					...
					//AfxMessageBox(_T("end CreateMatrix"));
			break;
		default:
			;//assert( false );
    ...
    acutPrintf( _T("\nCounters:") );
		...
		//AfxMessageBox(_T("End Serialize"));
	}
}	//Serialize(CArchive &ar)
...
void Controller::DoBOMSummary()
{
    ...
    bool bIsRoofProtected = CheckRoofProtectionIsValid( true );
		//return;
...
HGLOBAL hDialogTemplate = LoadResource(hInst, hResource);
		*/
		...
		//pBOMSummaryDialog = new BOMSummaryDialog(NULL,NULL, &siList);
		//pBOMSelectionDialog = new BOMSelectionDialog();		//iRet = pBOMSummaryDialog->DoModal();	
...
bool Controller::GetSideOfBayAndNumBoardsFromCommandLine( SideOfBayEnum &eSide, int &iNumberOfBoards, BayList *pBays, bool &bMatchToDecks )
{
	...
	case( RTKWORD ):			//(-5005) Keyword returned from getxxx() routine
		...
		retFlag= GetSideOfBayAndNumBoardsFromCommandLine( eSide, iNumberOfBoards, pBays, bMatchToDecks );
		break;
...
*/
	...
	if(sErrMsgG.GetLength()>0)
	{
		//MessageBeep(MB_ICONEXCLAMATION);
		//MessageBox( NULL, sErrMsgG, _T("Lapboard has no planks"), MB_OK );
		msgboxdlg m_pMsgBox = new msgboxdlg();
		m_pMsgBox.msg=sErrMsgG;
		m_pMsgBox.DoModal();
	}
	acedRestoreStatusBar();
}	//ApplyToSelection(Bay *pBay, BayList *pBays )
...
void Controller::SetViewportPosition(AcGePoint2d ptBottomLeft, AcGePoint2d ptTopRight)
{
    ...
    acedCommandS(RTSTR, _T("_ZOOM"), RTSTR, _T("_WINDOW"), RTSTR, sBL, RTSTR, sTR, 0);    // zoom to a reasonable spot
}

void Controller::StoreCurrentView()
{
    acedCommandS(RTSTR, _T("_VIEW"), RTSTR, _T("_SAVE"), RTSTR, VIEW_EDIT_STANDARD_CURRENT, 0);
}

void Controller::RestoreCurrentView()
{
    acedCommandS(RTSTR, _T("_VIEW"), RTSTR, _T("_RESTORE"), RTSTR, VIEW_EDIT_STANDARD_CURRENT, 0);
}
...
*/
	acedCommandS(RTSTR, _T("_MOVE"), RTSTR, _T(""), RTSTR, _T("\\"), RTNONE );
}
