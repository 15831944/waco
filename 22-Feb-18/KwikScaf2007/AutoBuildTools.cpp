...
//			m_LBTplt.m_bShowLapBoards		= false;
        ...
        sPrompt.Format( _T("\nSpecify next point of lapboard or change number of boards<%i>:"), iBoards );
				...
				p1stBaySelected = GetBayByInternalPoint(m_ptMousePoint[MP_2ND], eHit);
				if( p1stBaySelected==NULL )
				{
					MessageBeep(MB_ICONEXCLAMATION);
					MessageBox( NULL, _T("The second point of the lapboard must be within a bay!"),
								_T("Invalid Point"), MB_OK );
					continue;
				}
...
*/
	...
	;//assert( dDistance<=dSeparation+0.01 && dDistance>=dSeparation-0.01 );
