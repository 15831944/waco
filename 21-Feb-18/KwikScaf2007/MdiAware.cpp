...
//	EntityReactor		*pReactor;
  ...
  acutPrintf( _T("\nCommandEnded: %s"), cmdStr );
	...
	*/
				...
				{
					reactors = (AcDbVoidPtrArray*)pEnt->reactors();
					if (reactors != NULL)
					{
			 ...
			 ((A2.x-A1.x)/(A2.y-A1.y))>((B2.x-B1.x)/(B2.y-B1.y))-ROUND_ERROR_ANGLE_SMALL )
	{
		...
		else
		{
				...
				B1.x>A1.x-ROUND_ERROR )
			{
				...
				///;//assert( dC2<dTemp+ROUND_ERROR &&	dC2>dTemp-ROUND_ERROR );
					 ...
					 B2.x>A2.x-ROUND_ERROR )
			{
				...
				///;//assert( dC1<dTemp+ROUND_ERROR &&	dC1>dTemp-ROUND_ERROR );
			...
			else
			{
				dSlope1 = ((B1.y-A1.y)/(B1.x-A1.x));
				dSlope2 = ((B2.y-A2.y)/(B2.x-A2.x));
				///;//assert( dSlope1>dSlope2+ROUND_ERROR_ANGLE ||	dSlope1<dSlope2-ROUND_ERROR_ANGLE );
				...
				///;//assert( ptCentre.y<dTemp+ROUND_ERROR && ptCentre.y>dTemp-ROUND_ERROR );
			}
		}
	}
	else
