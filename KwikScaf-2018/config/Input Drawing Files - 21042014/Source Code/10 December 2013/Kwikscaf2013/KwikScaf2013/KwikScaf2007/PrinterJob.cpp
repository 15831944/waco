...
void CPrinterJob::OnFilePrint()
{
	...
	if (OnPreparePrinting(&printInfo))
	{
		...
		cs.Format(_T("Stampa di %s in corso .."), strTitle);
		...
		dlgPrintStatus.wndProg.SetRange((short)0, (short)iPageTot);
		...
		for (printInfo.m_nCurPage = nStartPage; printInfo.m_nCurPage != nEndPage; printInfo.m_nCurPage += nStep)
		{
			...
			wsprintf(szBuf, _T("%d %%"), iPrintedPages*100/iPageTot);
			...
			if (tck > 60)
			{
				...
				DWORD tckS = tck%60;
				tck /= 60;
				strTemp.Format(_T("%d minuti, %d secondi"), tck, tckS);
			}
			else
			{
				if (iPageTot > 1)
				{
					if (tck > 0) strTemp.Format(_T("%d secondi"), tck);
					else strTemp = _T("??");
				}
			}
