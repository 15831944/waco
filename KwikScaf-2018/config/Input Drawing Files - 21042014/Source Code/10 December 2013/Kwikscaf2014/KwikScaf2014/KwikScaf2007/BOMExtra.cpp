...
CString BOMExtraElement::GetCompDetailsDescription()
{
	...
	if( m_dLength>100.00 )
		sOutString.Format(_T("%s %s %.1fm"), sDesc, GetComponentMaterialStr( m_eMaterialType ), m_dLength*CONVERT_MM_TO_M );
	else
		sOutString.Format(_T("%s %s %.0f"), sDesc, GetComponentMaterialStr( m_eMaterialType ), m_dLength);

	//try to create the string again, using this part number
	assert( gpController!=NULL );
	sPartNo	= gpController->GetCompDetails()->GetComponentPartNumberStr( m_eSystem, m_eComponentType, m_dLength, m_eMaterialType );
...
void BOMExtra::ConvertBOMCompListToDialogList()
{
	...
	while (iIndex < iNumberOfComponents)
	{
		...
		pBOMComponent->m_dLength		= pComponent->GetLengthCommon();
...
void BOMExtra::ConvertDialogListToBOMCompList()
{
	...
	while (iIndex < iNumberOfItems)
	{
		...
		while ( iCount < abs(iNumberOfComponents) )
		{
			...
			pComponent->SetComponent(pBOMExtraComp->m_dLength, pBOMExtraComp->m_eComponentType, pBOMExtraComp->m_eMaterialType);
